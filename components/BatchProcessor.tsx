...
import { Level, Region, ArticleType, ReverseLogisticsMode, Gender, BusinessBuffers, Marketplace, Brand } from '../types';
...
const BatchProcessor: React.FC<BatchProcessorProps> = ({ 
  ...
  const results = useMemo(() => {
    ...
    // Ensure activeData is treated as an array to prevent "Property 'map' does not exist on type 'unknown'"
    ...
    return activeData.map(row => {
      ...
      if (currentMarketplace === Marketplace.AJIO) {
        ...
        const aisp = findAISPForTarget(target, level, row.articleType, false, Region.LOCAL, ReverseLogisticsMode.FIXED, 0, currentMarketplace, row.brand);
        breakdown = calculateBreakdown(aisp, level, row.articleType, false, Region.LOCAL, ReverseLogisticsMode.FIXED, 0, currentMarketplace, row.brand);
  ...
  const exportToExcel = () => {
    ...
    if (currentMarketplace === Marketplace.AJIO) {
      ...
      const headers = ['Brand*', 'ASIN*', 'Amazon sku*', 'Myntra sku*', 'Sku id*', 'Style id*', 'Gender*', 'Article type*', 'TP (Cost)*', ...bufferCols, 'Target Settlement', 'Seller Price (AISP)', 'GTA Fee (Logistics)', 'Customer Price', 'Commission % (Inc. GST)', 'Commission Amt (Inc. GST)', 'Fixed Fee (Excl. GST)', 'LEVEL', 'Fixed Fee GST AMT', 'TCS', 'TDS', 'Bank Settlement', 'Net Profit', 'ROI %'];
      wsData.push(headers);
      results.forEach(item => {
        const profit = round(item.totalActualSettlement - (item.baseTp || 0));
        const roi = (item.baseTp && item.baseTp > 0) ? round((profit / item.baseTp) * 100) : 0;
        ...
        if (isBuffersEnabled) row.push(round(item.baseTp * buffers.adsPercent / 100), round(item.baseTp * buffers.dealDiscountPercent / 100), round(item.baseTp * buffers.reviewPercent / 100), round(item.baseTp * buffers.profitMarginPercent / 100), round(item.baseTp * buffers.returnPercent / 100));
        row.push(round(item.targetSettlement), round(item.aisp), round(item.logisticsFee), round(item.customerPrice), `${item.commissionRate}%`, round(item.commission), round(item.fixedFee), item.level, round(item.fixedFee * GST_RATE), round(item.tcs || 0), round(item.tds || 0), round(item.totalActualSettlement), profit, `${roi}%`);
        wsData.push(row);
      });
    }
  ...
  // Fixed: Cast Object.values and specific entry to any[] to avoid "Property 'length' does not exist on type 'unknown'"
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6">
      ...
      <div className="w-full max-w-[1200px] grid grid-cols-1 lg:grid-cols-2 gap-6 px-2">
        ...
        <div className="bg-white rounded-3xl shadow-xl ring-1 ring-inset ring-white/40 border border-forest-accent/70 dark:bg-forest-pine/40 overflow-hidden flex flex-col transition-all">
          ...
          <div className={`p-4 grid grid-cols-2 gap-x-4 gap-y-4 transition-opacity duration-300 ${isBuffersEnabled ? 'opacity-100' : 'opacity-20 pointer-events-none'}`}>
            {Object.entries(buffers).map(([key, value]) => (
              <div key={key} className="space-y-1">
                <label className="text-[7px] font-black text-forest-pine/40 uppercase tracking-widest dark:text-forest-sage/60">{bufferLabelMap[key]}</label>
        ...
        <div className="w-full max-w-full px-2 pb-12 mt-4 overflow-x-auto">
           <div className="bg-white rounded-3xl border border-forest-accent shadow-xl ring-1 ring-white/60 dark:bg-forest-pine/40 dark:border-forest-leaf/20 overflow-hidden">
             <div className="overflow-x-auto min-w-full">
               <table className="w-full text-[9px] text-left border-collapse min-w-[2200px]">
                 <thead className="bg-forest-mint/90 dark:bg-forest-pine/80 border-b border-forest-accent">
                   <tr className="divide-x divide-forest-accent/30">
                     ...
                     <th className="px-4 py-3 font-black uppercase text-forest-pine tracking-widest dark:text-forest-mint bg-forest-mint/50">Target Payout</th>
                     ...
                     <th className="px-4 py-3 font-black uppercase text-forest-pine tracking-widest dark:text-forest-mint">Comm % (Inc. GST)</th>
                     <th className="px-4 py-3 font-black uppercase text-forest-pine tracking-widest dark:text-forest-mint">Logistics</th>
                     <th className="px-4 py-3 font-black uppercase text-forest-pine tracking-widest dark:text-forest-sage text-right">ROI Efficiency</th>
                   </tr>
                 </thead>
                   ...
                   {results.map((row, idx) => {
                     ...
                     return (
                       <tr key={idx} className="hover:bg-forest-mint/80 dark:hover:bg-forest-pine/40 transition-all divide-x divide-forest-accent/10">
                         ...
                         <td className="px-4 py-2.5 text-forest-pine/60 dark:text-forest-sage/60">₹{row.logisticsFee}</td>
